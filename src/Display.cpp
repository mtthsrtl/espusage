...
static bool touchReadRegister(uint16_t reg, uint8_t *data, size_t length) {
  ...
  if (Wire.requestFrom(touchAddress, (uint8_t)length) != length) return false;
  ...
  if (touchAddress && now - touchLastPollMs >= 16) {
    ...
    if (!touchReadRegister(GT911_TOUCH_STATE, &state, 1)) {
      ...
      if (state & 0x80) {
        ...
        if (count > 0 && count <= 5) {
          uint8_t point[8] = {};
          if (touchReadRegister(GT911_FIRST_POINT, point, sizeof(point))) {
        ...
        touchWriteRegister(GT911_TOUCH_STATE, 0);
