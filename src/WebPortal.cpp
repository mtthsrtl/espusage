...
function usageValue(w){if(w.monetary){let c=w.currency||'let root=q(id),max=Math.max(0,...buckets.map(b=>b.valid?Number(b[field]||0):0));root.replaceChildren();buckets.forEach(b=>{let bar=document.createElement('span'),v=Number(b[field]||0);bar.style.height=(b.valid&&max?Math.max(2,Math.round(34*v/max)):2)+'px';bar.style.opacity=b.valid?'1':'.25';bar.title=b.valid?String(v):'No sample';root.append(bar)})}
...
,u=Number(w.used_amount||0),l=Number(w.limit_amount);return l>=0?`${c}${u.toFixed(2)} / ${c}${l.toFixed(2)}`:`${c}${u.toFixed(2)}`}return w.used_percent<0?'--':w.used_percent.toFixed(1)+'%'}
