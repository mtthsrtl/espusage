...
fetch('/api/status').then(r=>r.json()).then(s=>{q('[name=background_color]').value=s.background_color||'#000000';let five=q('#usageRows option[value=codex_5h]');if(five)five.selected=!!s.show_codex_5h;if(q('#buildVersion'))q('#buildVersion').textContent=s.version||'0.8.25.1712'});
...
void webBegin(AppConfig &c, bool setupMode){
 ...
 server.on("/api/status",HTTP_GET,[]{
   ...
   d["display_style"]=cfg->displayStyle==2?"telemetry":cfg->displayStyle==1?"open":"panels";
 ...
 server.on("/api/config",HTTP_POST,[]{
   ...
   cfg->displayStyle=server.arg("display_style")=="telemetry"?2:server.arg("display_style")=="open"?1:0; cfg->displayAvailable=server.arg("display_mode")=="remaining";
   ...
   Serial.printf("[config][nvs] Display style=%s, mode=%s, rows: Cursor=%u/%u/%u/%u, Codex=%u/%u/%u\n",cfg->displayStyle==2?"telemetry":cfg->displayStyle==1?"open":"panels",cfg->displayAvailable?"remaining":"used",cfg->showCursorModels,cfg->showCursorOther,cfg->showCursorOnDemand,cfg->showCursorThirtyMinute,cfg->showCodexFiveHour,cfg->showCodexWeekly,cfg->showCodexThirtyMinute);
