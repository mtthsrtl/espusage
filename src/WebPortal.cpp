...
<div class=advanced><form method=post action=/api/config><div class=grid><section><h2>Codex</h2><p class=warn>Requires a trusted adapter; no official consumer Usage API.</p><label><input type=checkbox name=codex_on> Enabled</label><label>HTTPS adapter URL</label><input name=codex_url><label>Adapter bearer token</label><input type=password name=codex_tok></section>
...
fetch('/api/status').then(r=>r.json()).then(s=>{if(s.setup_mode){document.body.classList.add('setup');q('#intro').textContent='Setup mode: choose your WiFi network to continue.'}else if(s.ssid){q('#manual').value=s.ssid}});scan();
...
void webBegin(AppConfig &c, bool setupMode){
 ...
 server.on("/api/status",HTTP_GET,[]{JsonDocument d;d["firmware"]="espusage";d["setup_mode"]=isSetupMode;d["ssid"]=cfg->wifiProvisioned?cfg->wifiSsid:"";d["ip"]=isSetupMode?WiFi.softAPIP().toString():WiFi.localIP().toString();d["rssi"]=isSetupMode?0:WiFi.RSSI();d["uptime_ms"]=millis();d["free_heap"]=ESP.getFreeHeap();d["psram_free"]=ESP.getFreePsram();d["codex_configured"]=cfg->codex.enabled;d["cursor_configured"]=cfg->cursor.enabled;d["warning_percent"]=cfg->warningPercent;d["critical_percent"]=cfg->criticalPercent;String s;serializeJson(d,s);server.send(200,"application/json",s);});
 ...
 server.on("/api/config",HTTP_POST,[]{cfg->hostname=server.arg("host");cfg->codex.enabled=server.hasArg("codex_on");cfg->codex.endpoint=server.arg("codex_url");if(server.arg("codex_tok").length())cfg->codex.token=server.arg("codex_tok");cfg->cursor.enabled=server.hasArg("cursor_on");cfg->cursor.endpoint=server.arg("cursor_url");if(server.arg("cursor_tok").length())cfg->cursor.token=server.arg("cursor_tok");cfg->brightness=constrain(server.arg("bright").toInt(),1,100);cfg->refreshMinutes=constrain(server.arg("refresh").toInt(),1,1440);cfg->warningPercent=constrain(server.arg("warning").toInt(),1,98);cfg->criticalPercent=constrain(server.arg("critical").toInt(),cfg->warningPercent+1,100);if(!saveConfig(*cfg)){server.send(500,"text/plain","Could not save settings.");return;}restartAfterResponse("Settings saved. Restarting...");});
