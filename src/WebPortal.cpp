...
<div class=advanced><section><h2>Live usage</h2><p>Current provider values and the last 30 minutes. This view contains no access tokens or account identifiers.</p><div class=grid><div class=usage-card><h3>Cursor</h3><div class=usage-main id=cursorMain>Waiting...</div><div class=mini id=cursorChart></div><div class=usage-detail id=cursorDetail></div></div><div class="usage-card codex"><h3>Codex</h3><div class=usage-main id=codexMain>Waiting...</div><div class=mini id=codexChart></div><div class=usage-detail id=codexDetail></div></div></div></section>
...
function renderCodex(c){let r=c.recent_30m||{},valid=(r.buckets||[]).filter(b=>b.valid).length;q('#codexMain').textContent=!r.available?'NO DATA':r.ready?`+${Number(r.delta_pp||0).toFixed(2)} PP`:`COLLECTING ${valid}/6`;q('#codexDetail').textContent=`5-hour ${c.primary.used_percent<0?'--':c.primary.used_percent.toFixed(1)}% used | Weekly ${c.secondary.used_percent<0?'--':c.secondary.used_percent.toFixed(1)}% used\nSix 5-minute buckets from the weekly limit change.\nHistory is kept in RAM and restarts after reboot or a limit reset.\n${r.status||c.status}`;chart('#codexChart',r.buckets||[],'delta_pp')}
...
void webBegin(AppConfig &c, bool setupMode){
 ...
 server.on("/api/status",HTTP_GET,[]{
   ...
   d["show_codex_30m"]=cfg->showCodexThirtyMinute; d["brightness"]=cfg->brightness; d["refresh_minutes"]=cfg->refreshMinutes;
 ...
 server.on("/api/config",HTTP_POST,[]{
   ...
   cfg->showCodexFiveHour=server.hasArg("show_codex_5h"); cfg->showCodexWeekly=server.hasArg("show_codex_weekly"); cfg->showCodexThirtyMinute=server.hasArg("show_codex_30m");
   ...
   Serial.printf("[config][nvs] Display style=%s, rows: Cursor=%u/%u/%u/%u, Codex=%u/%u/%u\n",cfg->displayStyle==1?"open":"panels",cfg->showCursorModels,cfg->showCursorOther,cfg->showCursorOnDemand,cfg->showCursorThirtyMinute,cfg->showCodexFiveHour,cfg->showCodexWeekly,cfg->showCodexThirtyMinute);
