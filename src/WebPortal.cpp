...
fetch('/api/status').then(r=>r.json()).then(s=>{q('[name=background_color]').value=s.background_color||'#000000';let five=q('#usageRows option[value=codex_5h]');if(five)five.selected=!!s.show_codex_5h;if(q('#buildVersion'))q('#buildVersion').textContent=s.version||'0.8.25.1445'});
