...
fetch('/api/status').then(r=>r.json()).then(s=>{q('[name=background_color]').value=s.background_color||'#000000';let five=q('#usageRows option[value=codex_5h]');if(five)five.selected=!!s.show_codex_5h;if(q('#buildVersion'))q('#buildVersion').textContent=s.version||'0.9.6.1332'});
...
void webBegin(AppConfig &c, bool setupMode, UsageRefreshHandler refreshHandler){
 ...
 server.on("/api/tokens",HTTP_POST,[]{
   if(cfg->tokenApiKey.length()<12){server.send(403,"application/json","{\"ok\":false,\"error\":\"token API key is not configured\"}");return;}
   String apiKey=server.arg("api_key");
   if(apiKey!=cfg->tokenApiKey){server.send(401,"application/json","{\"ok\":false,\"error\":\"invalid API key\"}");return;}
