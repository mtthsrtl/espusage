...
static UsageRefreshHandler requestUsageRefresh;
...
static bool authorizeSensitiveApi() {
  if (!cfg || cfg->tokenApiKey.length() < 12) {
    server.send(403,"application/json","{\"ok\":false,\"error\":\"API key is not configured\"}");
    return false;
  }
  if (server.arg("api_key") != cfg->tokenApiKey) {
...
<div class=advanced><section><h2>Live usage</h2><p>Current provider values and activity for the last 30 minutes. This view contains no access tokens or account identifiers.</p><div class=grid><div class=usage-card><h3>Cursor</h3><div class=usage-main id=cursorMain>Waiting...</div><div class=mini id=cursorChart></div><div class=usage-detail id=cursorDetail></div></div><div class="usage-card codex"><h3>Codex</h3><div class=usage-main id=codexMain>Waiting...</div><div class=mini id=codexChart></div><div class=usage-detail id=codexDetail></div></div></div></section>
  <form method=post action=/api/config><div class=settings-grid><div><section><h2>Codex</h2><p class=warn>Direct Codex limit usage is unofficial and the access token expires.</p><label><input type=checkbox name=codex_on> Enabled</label><label class=switch><input type=checkbox name=codex_credits_fallback><span>Show credit balance when a limit is exhausted</span></label><p>An exhausted 5-hour or weekly row switches to credits and automatically returns after its limit resets.</p><label>Codex access_token (empty keeps current)</label><input type=password name=codex_tok autocomplete=new-password><p>PowerShell: copy the local Codex access token:</p><pre class=command>(Get-Content "$env:USERPROFILE\.codex\auth.json" -Raw | ConvertFrom-Json).tokens.access_token | Set-Clipboard</pre><label>ChatGPT account_id, optional (empty keeps current)</label><input type=password name=codex_acct autocomplete=off><p>PowerShell: copy the matching account ID:</p><pre class=command>(Get-Content "$env:USERPROFILE\.codex\auth.json" -Raw | ConvertFrom-Json).tokens.account_id | Set-Clipboard</pre><label>Token webhook API key (empty keeps current)</label><input type=password name=token_api_key autocomplete=new-password minlength=12><p>Required by <code>POST /api/tokens</code>. Use at least 12 characters and keep it private.</p><label>Custom limit adapter URL (empty uses Codex app token)</label><input name=codex_url></section>
  <section><h2>Cursor personal</h2><p class=warn>Unofficial read-only dashboard API; may change.</p><label><input type=checkbox name=cursor_on> Enabled</label><label>Usage URL</label><input name=cursor_url value="https://cursor.com/api/usage-summary"><label>Cursor access token (empty keeps current)</label><input type=password name=cursor_tok autocomplete=new-password><p>PowerShell (uses Python included on this computer):</p><pre class=command>python -c 'import os,sqlite3; db=os.path.join(os.environ["APPDATA"],r"Cursor\User\globalStorage\state.vscdb"); print(sqlite3.connect(db).execute("SELECT value FROM ItemTable WHERE key=''cursorAuth/accessToken''").fetchone()[0])' | Set-Clipboard</pre><p>Treat copied tokens as account credentials and use them only on a trusted network.</p><button>Save provider settings &amp; restart</button></section><section><h2>Build</h2><p>Version <strong id=buildVersion>0.9.7.1713</strong></p><p>Beta · build date and time use CEST.</p></section></div>
...
async function loadTouch(){try{let r=await fetch('/api/touch',{cache:'no-store'}),t=await r.json(),working=t.point_frames>0||t.tap_events>0;q('#touchMain').textContent=t.status;q('#touchMain').className='usage-main '+(working?'ok':'bad');q('#touchDetail').textContent=`Controller: ${t.controller}${t.address?' @ '+hx(t.address):''}\nI2C: SDA ${t.sda}, SCL ${t.scl}, 50 kHz | devices: ${t.bus_devices}\nDisplay: RGB 480 x 480 | pixel clock: 10 MHz\nRaw size: ${t.raw_width} x ${t.raw_height}\nLVGL callbacks: ${t.callback_calls} | fallback reads: ${t.fallback_reads} | GT911 polls: ${t.polls}\nState reads: ${t.state_reads} | errors: ${t.state_read_errors} | last state: ${hx(t.last_state)} | touches: ${t.last_touch_count}\nReady frames: ${t.ready_frames} | point frames: ${t.point_frames} | ACK errors: ${t.ack_errors}\nLast point raw: ${t.raw_x}, ${t.raw_y} | display: ${t.display_x}, ${t.display_y} | pressed: ${t.pressed}\nGestures down/up/tap/toggle: ${t.down_events}/${t.up_events}/${t.tap_events}/${t.toggle_events}\nLast event: ${t.last_event} at ${t.last_event_ms} ms\nLast error: ${t.last_error}\nPossible GSL3680 at 0x40: ${t.possible_gsl3680?'yes':'no'} | view: ${t.remaining_view?'remaining':'used'}`;}catch(e){q('#touchMain').textContent='Touch diagnostics unavailable';q('#touchMain').className='usage-main bad'}}
  fetch('/api/status').then(r=>r.json()).then(s=>{if(s.setup_mode){document.body.classList.add('setup');q('#intro').textContent='Setup mode: choose your WiFi network to continue.'}else if(s.ssid){q('#manual').value=s.ssid}q('[name=codex_on]').checked=!!s.codex_configured;q('[name=codex_credits_fallback]').checked=!!s.codex_credits_fallback;q('[name=cursor_on]').checked=!!s.cursor_configured;q('[name=codex_url]').value=s.codex_endpoint||'';q('[name=cursor_url]').value=s.cursor_endpoint||'https://cursor.com/api/usage-summary';q('[name=display_style]').value=s.display_style||'panels';q('[name=display_mode]').value=s.display_mode||'used';let rows={cursor_models:s.show_cursor_models,cursor_other:s.show_cursor_other,cursor_ondemand:s.show_cursor_ondemand,cursor_30m:s.show_cursor_30m,codex_weekly:s.show_codex_weekly,codex_30m:s.show_codex_30m};[...q('#usageRows').options].forEach(o=>o.selected=!!rows[o.value]);q('[name=pace_indicator_color]').value=s.pace_indicator_color||'#FFFFFF';q('[name=pace_indicator_glow_color]').value=s.pace_indicator_glow_color||s.pace_indicator_color||'#FFFFFF';q('[name=pace_indicator_glow]').checked=!!s.pace_indicator_glow;q('[name=display_off_enabled]').checked=!!s.display_off_enabled;q('[name=display_off_from]').value=s.display_off_from||'22:00';q('[name=display_off_until]').value=s.display_off_until||'07:00';q('[name=automatic_reboot_enabled]').checked=!!s.automatic_reboot_enabled;q('[name=automatic_reboot_hours]').value=s.automatic_reboot_hours||2;q('[name=host]').value=s.hostname||'espusage';q('[name=bright]').value=s.brightness||85;q('[name=refresh]').value=s.refresh_minutes||5;q('[name=overpace_color]').value=s.overpace_color||'#DDF542';q('[name=warning_color]').value=s.warning_color||'#F0A020';q('[name=warning]').value=s.warning_percent||70;q('[name=critical]').value=s.critical_percent||90;if(s.setup_mode||page==='wifi')scan();if(page==='debug'){loadUsage();loadTouch();setInterval(loadUsage,15000);setInterval(loadTouch,2000)}}).catch(()=>{q('#intro').textContent='Device status unavailable. Reload the page.'});
fetch('/api/status').then(r=>r.json()).then(s=>{q('[name=background_color]').value=s.background_color||'#000000';let five=q('#usageRows option[value=codex_5h]');if(five)five.selected=!!s.show_codex_5h;if(q('#buildVersion'))q('#buildVersion').textContent=s.version||'0.9.7.1713'});
...
static void addSnapshotJson(JsonObject target, const UsageSnapshot &snapshot) {
  ...
  target["credits"] = snapshot.credits;
...
void webBegin(AppConfig &c, bool setupMode, UsageRefreshHandler refreshHandler){
 cfg=&c; isSetupMode=setupMode; requestUsageRefresh=refreshHandler;
 ...
 server.on("/api/status",HTTP_GET,[]{
   ...
   d["codex_credits_fallback"]=cfg->codexCreditsFallback;
   d["token_api_configured"]=cfg->tokenApiKey.length()>=12;
   ...
   d["display_style"]=cfg->displayStyle==4?"vertical":cfg->displayStyle==3?"matrix":cfg->displayStyle==2?"telemetry":cfg->displayStyle==1?"open":"panels";
   ...
   d["automatic_reboot_enabled"]=cfg->automaticRebootEnabled;
   d["automatic_reboot_hours"]=cfg->automaticRebootHours;
 ...
 auto handleUsageRefresh=[](){
   if(!requestUsageRefresh){server.send(503,"application/json","{\"ok\":false,\"error\":\"refresh unavailable\"}");return;}
   requestUsageRefresh();
   server.sendHeader("Cache-Control","no-store");
   server.send(202,"application/json","{\"ok\":true,\"status\":\"refresh_queued\"}");
 };
 server.on("/api/usage/refresh",HTTP_GET,handleUsageRefresh);
 server.on("/api/usage/refresh",HTTP_POST,handleUsageRefresh);
 server.on("/api/tokens",HTTP_POST,[]{
   if(!authorizeSensitiveApi())return;
   String codexToken=server.arg("codex_token"),accountId=server.arg("codex_account_id"),cursorToken=server.arg("cursor_token");
   codexToken.trim(); accountId.trim(); cursorToken.trim();
   if(codexToken.startsWith("Bearer "))codexToken.remove(0,7);
   bool updateCodex=codexToken.length()>0,updateAccount=accountId.length()>0,updateCursor=cursorToken.length()>0;
   if(!updateCodex&&!updateAccount&&!updateCursor){server.send(400,"application/json","{\"ok\":false,\"error\":\"no token values supplied\"}");return;}
   if(codexToken.length()>4096||cursorToken.length()>4096||accountId.length()>256){server.send(413,"application/json","{\"ok\":false,\"error\":\"credential value too long\"}");return;}
   if(updateCodex)cfg->codex.token=codexToken;
   if(updateAccount)cfg->codex.accountId=accountId;
   if(updateCursor)cfg->cursor.token=cursorToken;
   if(!saveConfig(*cfg)){server.send(500,"application/json","{\"ok\":false,\"error\":\"could not save credentials\"}");return;}
   if(requestUsageRefresh)requestUsageRefresh();
   JsonDocument d;d["ok"]=true;d["codex_token_updated"]=updateCodex;d["codex_account_updated"]=updateAccount;d["cursor_token_updated"]=updateCursor;d["refresh_queued"]=requestUsageRefresh!=nullptr;
   String s;serializeJson(d,s);server.sendHeader("Cache-Control","no-store");server.send(200,"application/json",s);
 });
 server.on("/api/reboot",HTTP_POST,[]{
   if(!authorizeSensitiveApi())return;
   server.sendHeader("Cache-Control","no-store");
   server.send(200,"application/json","{\"ok\":true,\"status\":\"restarting\"}");
   Serial.println("[system][api] Authenticated reboot requested");
   delay(500);
   ESP.restart();
 });
 ...
 server.on("/api/config",HTTP_POST,[]{
   ...
   cfg->codexCreditsFallback=server.hasArg("codex_credits_fallback");
   String tokenApiKey=server.arg("token_api_key");tokenApiKey.trim();if(tokenApiKey.length()>=12)cfg->tokenApiKey=tokenApiKey;
   ...
   cfg->displayStyle=server.arg("display_style")=="vertical"?4:server.arg("display_style")=="matrix"?3:server.arg("display_style")=="telemetry"?2:server.arg("display_style")=="open"?1:0; cfg->displayAvailable=server.arg("display_mode")=="remaining";
   ...
   cfg->automaticRebootEnabled=server.hasArg("automatic_reboot_enabled");
   cfg->automaticRebootHours=constrain(server.arg("automatic_reboot_hours").toInt(),1,168);
   ...
   Serial.printf("[config][nvs] Display style=%s, mode=%s, rows: Cursor=%u/%u/%u/%u, Codex=%u/%u/%u\n",cfg->displayStyle==4?"vertical":cfg->displayStyle==3?"matrix":cfg->displayStyle==2?"telemetry":cfg->displayStyle==1?"open":"panels",cfg->displayAvailable?"remaining":"used",cfg->showCursorModels,cfg->showCursorOther,cfg->showCursorOnDemand,cfg->showCursorThirtyMinute,cfg->showCodexFiveHour,cfg->showCodexWeekly,cfg->showCodexThirtyMinute);
